...
public abstract class User : IUser
{
    ...
    public UserOperationResult CreateEntity(EntityType type, string entityId, string? entityDescription = null)
    {
        switch (type)
        {
            case EntityType.Project:
                if (!UserPermissions.Contains(Permission.CanCreateProject))
                {
            ...
            case EntityType.Task:
                if (!UserPermissions.Contains(Permission.CanCreateTask))
                {
        ...
        object? propertyValue)
    {
        ...
        {
            case EntityType.Project:
                if (!UserPermissions.Contains(Permission.CanChangeProjectProperty))
                {
                ...
                break;
            case EntityType.Task:
                if (!UserPermissions.Contains(Permission.CanChangeTaskProperty))
                {
                    AppendToReport("Not enough permissions to change a project property");
                    return UserOperationResult.Failed;
                }
    ...
    public UserOperationResult DeleteEntity(EntityType type, string entityId)
    {
        ...
        {
            case EntityType.Project:
                if (!UserPermissions.Contains(Permission.CanDeleteProject))
                {
                ...
                break;
            case EntityType.Task:
                if (!UserPermissions.Contains(Permission.CanDeleteTask))
                {
